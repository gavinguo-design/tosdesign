...
export async function onRequestPost({ request }) {
  ...
  const suffix = isAmbient
    ...
    ? '，为 1:1 桌面小组件制作的全幅无边背景素材。画面必须从左、右、上、下四条边连续延展至边缘（full-bleed），不得生成卡片、App 图标、圆角方框、相框、内嵌矩形、留白边框、金属边框、阴影底座或任何 UI 容器。要求：真实风景摄影/自然景观质感（如真实天空、云层、阳光镜头光斑、海面/山峰/城市天际线），摄影级真实质感，非插画非漫画。构图：画面左侧/左上角预留深色或深化阴影区域（用于叠加白色文字），整体色调与主题呼应（晴天=暖金蓝天，夜晚=深蓝星空，雨天=灰蓝阴沉，日落=橙粉斑烂）。严禁出现卡通风格/emoji风格图标、手绘贴纸式小图标（如卡通太阳笑脸、手绘云朵、平涂描边图形），不要求无图形元素，但必须是摄影级/自然写实，不能是卡通插画风格。无任何文字，画面整体通透明亮'
